...
from sample_personal_qc_inference import predict_image_paths  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--qc-batch-size", type=int, default=32)
    parser.add_argument("--qc-num-workers", type=int, default=4)
...
def build_augmented_dataset(
    ...
    qc_device: str,
    qc_image_size: int,
    qc_apply_eyeq_preprocess: bool,
    qc_preprocess_size: int,
    clahe_prob: float,
    ...
    for source_index, base_row in enumerate(base_rows):
        ...
        try:
            while len(accepted_rows) < target_augmented and attempts < max_attempts:
                batch_size = min(policy.generation_batch_size, max_attempts - attempts)
                candidates = generate_candidate_batch(
                    source_image=source_image,
                    base_row=base_row,
                    source_index=source_index,
                    attempt_start=attempts,
                ...
                qc_predictions = predict_image_paths(
                    image_paths=[candidate["image_path"] for candidate in candidates],
                    checkpoint_path=qc_checkpoint,
                    threshold=qc_threshold,
                    batch_size=qc_batch_size,
                    num_workers=qc_num_workers,
                    device=qc_device,
                    image_size=qc_image_size,
                    apply_eyeq_preprocess=qc_apply_eyeq_preprocess,
                    preprocess_size=qc_preprocess_size,
                )
                for candidate, prediction in zip(candidates, qc_predictions, strict=True):
                    ...
                    if prediction["pred_label"] == 0 and len(accepted_rows) < target_augmented:
        ...
        summary_counter[f"attempts_{base_row['aug_level']}"] += attempts
        if len(accepted_rows) < target_augmented:
            summary_counter[f"shortfall_{base_row['aug_level']}"] += target_augmented - len(accepted_rows)
...
def main() -> None:
    ...
    selected_rows, rejected_rows, summary_counter = build_augmented_dataset(
        base_rows=base_rows,
        image_dir=image_dir,
        qc_checkpoint=args.qc_checkpoint,
        qc_threshold=args.qc_threshold,
        qc_batch_size=args.qc_batch_size,
        qc_num_workers=args.qc_num_workers,
        qc_device=args.qc_device,
        qc_image_size=args.qc_image_size,
        qc_apply_eyeq_preprocess=args.qc_apply_eyeq_preprocess,
        qc_preprocess_size=args.qc_preprocess_size,
