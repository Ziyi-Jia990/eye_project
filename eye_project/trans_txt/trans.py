...
TAG_ALIAS_MAP = {
    "糖尿病视网膜病变轻度非增殖期": "糖尿病视网膜病变轻度非增生期",
    "糖尿病视网膜病变中度非增殖期": "糖尿病视网膜病变中度非增生期",
    "糖尿病视网膜病变重度非增殖期": "糖尿病视网膜病变重度非增生期",
    "糖尿病视网膜病变增殖期": "糖尿病视网膜病变增生期",
}
...
DIAGNOSIS_PRIORITY_MAP = {tag: idx for idx, tag in enumerate(DIAGNOSIS_PRIORITY)}
...
def normalize_tag_name(tag):
    tag = normalize_str(tag)
    if not tag:
        return ""

    tag = tag.replace("（", "(").replace("）", ")")
    tag = re.sub(r"\s+", "", tag)
    return TAG_ALIAS_MAP.get(tag, tag)
...
def canonicalize_diagnosis_tags(tags):
    normalized = [normalize_tag_name(tag) for tag in tags]
    normalized = [tag for tag in normalized if tag]
    normalized = dedup_keep_order(normalized)

    indexed = list(enumerate(normalized))
    indexed.sort(
        key=lambda item: (
            DIAGNOSIS_PRIORITY_MAP.get(item[1], len(DIAGNOSIS_PRIORITY_MAP) + item[0]),
            item[0],
        )
    )
    return [tag for _, tag in indexed]
...
def parse_abnormal_dr_tag(abnormal_list, eye_side):
    ...
    for item in abnormal_list:
        ...
        if side_text == "右眼" and eye_side == "right":
            result.append(normalize_tag_name(tag_name))
...
def parse_evisionai_tag_text(raw_text):
    ...
    cleaned = []
    for item in items:
        item = normalize_tag_name(item).strip(',')
...
def generate_clinical_description(file_path, output_file):
    ...
    for _, row in df.iterrows():
        ...
        tags_list = [normalize_tag_name(x) for x in parse_tags(row.get('tags', 0))]
        tags_list = [tag for tag in tags_list if tag]
        tags_list = dedup_keep_order(tags_list)
        ...
        diagnosis_final = canonicalize_diagnosis_tags(disease_list)
